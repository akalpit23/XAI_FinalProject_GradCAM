...
# add_bg_from_local("path_to_your_background_image.png")
...
# Sidebar inputs
st.sidebar.header("Step 1: Model Input")
...
# Sidebar for output file path
st.sidebar.header("Step 2: Output File Path")
output_file_path = st.sidebar.text_input(
    "Enter the output file path for the .dae file:",
    value="output.dae",
)

# Sidebar for input type selection
st.sidebar.header("Step 3: Choose Input Type")
...
st.sidebar.header("Step 4: Dimensionality Reduction Algorithm")
...
class HuggingFaceEmbeddingViz:
    ...
    def get_model_embeddings(self, text_list):
        """Extract embeddings for the provided text list.
    ...
    def generate_3d_visualization(
        ...
        # Helper function to get the current object
        def get_current_object():
            obj = bpy.context.active_object or bpy.context.object
            if obj is not None:
                return obj
            else:
                raise RuntimeError("No active object found in context.")
...
if input_type == "Words":
    ...
    if len(selected_domains) > 5:
        ...
        if st.sidebar.button("Generate Embeddings and Export"):
    ...
    if st.sidebar.button("Process Text and Export"):
    ...
    if df is not None:
        # Select the column containing text data in sidebar
        ...
        # Select the column containing domain labels in sidebar
        domain_column = st.sidebar.selectbox(
        ...
        if st.sidebar.button("Process CSV and Export"):
            try:
